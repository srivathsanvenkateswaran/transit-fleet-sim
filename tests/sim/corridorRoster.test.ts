...
describe('the multi-day roster (§3, §11.3)', () => {
  it('dispatches by departure rather than by spread: one duty per departure per service date', async () => {
    ...
    // Three daily departures over a three-day window, plus the weekend
    // relief on the two service dates it runs. `BUSES_PER_ROUTE` does not
    // apply and no configuration says how many coaches there are - the fleet
    // size falls out of the roster (§3.5).
    expect(simulation.rosteredCount).toBe(11)
