...
| **Metro occupancy** | GTFS-RT has `occupancy_status`, and buses populate it (section 8.9) - a metro run does not, because the seven-level ladder needs boardings, alightings and a headway to draw from, and this simulator has none of the three for a train. |
...
| Metro occupancy, alerts, fares, payment | Section 2.4. Bus occupancy is in scope (section 8.9). |
...
### 8.9 Occupancy: a consequence of the trip, not a number per request

`src/sim/occupancy.ts` turns a bus's position on its route, the time of day and
its direction into a load, rather than drawing a fresh figure on a timer. Four
inputs, each grounded in how a corridor's ridership actually behaves:

- **Where.** Every route has a demand "centre" - a deterministic
  per-route/per-direction fraction of route length standing in for the
  CBD-bound corridor most BMTC routes run through. Load rises approaching it
  and falls leaving it, so a bus fills and empties along the route instead of
  being flat for the whole trip.
- **When.** A two-peak time-of-day curve (morning, evening) with a gentle
  midday shoulder and a low overnight floor.
- **Which way.** A direction multiplier that favours the centre-bound
  direction in the morning and flips to favour the reverse in the evening, so
  an inbound morning bus and the same bus outbound at the same hour read
  differently.
- **Which vehicle.** Seated and standing capacity are modelled separately and
  differ by service class: an ordinary bus (42 seated, 38 standing) against a
  `KIA-*` Vayu Vajra airport coach (45 seated, 8 standing, almost no standing
  room by design). `STANDING_ROOM_ONLY` is the seated-capacity boundary, not a
  point on a shared percentage scale, so the two classes cross it at different
  loads.

A small per-route popularity multiplier and a per-lap perturbation, both keyed
through `rand`, sit on top of that shape - noise as a perturbation on the
demand model, never as the source of the number. The whole ladder, including
`FULL`, is reachable; reaching it takes a busy route, a lucky lap, peak time,
the aligned direction and standing near the centre all at once, which is what
keeps it the exception rather than the default reading.

The honesty rule already governing `tracking.state` extends unchanged: a dark
or untracked vehicle reports `occupancy_status: NO_DATA_AVAILABLE` and no
percentage field at all, and a stale fix's occupancy is computed against that
fix's own position and time - not against where the bus's simulated cursor has
since moved on to, and not against the serve time. Metro carries no occupancy;
`src/sim/metro.ts` states why at the point it would otherwise appear.
...
| **Bus occupancy** | Real in mechanism, not in measurement. `occupancy_status`/`occupancy_percentage` follow the honesty rule the rest of the service already keeps: a dark or untracked bus reports `NO_DATA_AVAILABLE` with no percentage at all, and a stale fix carries stale occupancy rather than a figure computed against wherever the bus has since moved. Load is a function of the trip - route position, time of day, direction, vehicle class - not a number redrawn on a timer. | §8.9 |
...
| **Bus occupancy demand** | The route "centre" a route's demand rises toward and falls away from is a deterministic per-route guess, not a real land-use or ridership survey - Bengaluru's actual busiest corridors were not measured. The two-peak time-of-day curve and the seated/standing capacities (§8.9) are plausible constants, not fitted to APC data because none exists for this project. | The shape is honest in kind - a bus does fill toward a centre and empty after it, morning and evening peaks are real, `FULL` is genuinely rarer than the rest of the ladder - and wrong in the specifics. A consuming app must not tune a threshold to these exact numbers, the same caveat as the prediction band above. |
| **Metro occupancy, alerts, fares** | Absent. Metro `occupancy_status` is never populated - `src/sim/metro.ts` explains why at the point it would otherwise appear - and no `Alert` entity is produced. | An app cannot demonstrate train crowding or disruption against this service. |
